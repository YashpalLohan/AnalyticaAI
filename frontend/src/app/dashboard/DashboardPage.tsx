...
export default function DashboardPage() {
  ...
  return (
    <div className="min-h-screen bg-cream flex flex-col">
      ...
      <nav className="bg-cream border-b border-border sticky top-0 z-50">
        <div className="max-w-content mx-auto px-6 h-[60px] flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            <span className="label text-ink-faint">{user?.email}</span>
            <button
              onClick={logout}
      ...
      <main className="flex-1 max-w-content mx-auto w-full px-6 py-12">

        {/* ── Header ── */}
        <div className="mb-12 border-b border-border pb-10">
          <p className="label-red mb-3">Dashboard</p>
          <h1 className="text-4xl font-black uppercase tracking-tight text-ink leading-none">
            Welcome,<br />
            <span className="text-red">{user?.full_name?.toUpperCase()}.</span>
          </h1>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-border mb-12">
          {kpis.map(k => (
            <div key={k.label} className="bg-cream p-6">
        ...
        <div className="mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-border">
            {quickActions.map(a => (
              <button
                key={a.tag}
                className="bg-cream p-6 text-left flex items-center justify-between
                           hover:bg-ink hover:text-cream group transition-colors duration-100"
              >
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 border border-border flex items-center justify-center
                                  group-hover:border-border-dark transition-colors">
                    <a.icon size={16} className="text-ink-muted group-hover:text-cream transition-colors" />
                  </div>
                  <div>
                    <p className="text-sm font-bold uppercase tracking-wide text-ink group-hover:text-cream transition-colors">
                      {a.label}
                    </p>
                    <p className="text-xs text-ink-faint group-hover:text-cream/60 transition-colors mt-0.5">
                      {a.desc}
                    </p>
                  </div>
                </div>
                <ChevronRight size={14} className="text-ink-faint group-hover:text-red transition-colors" />
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="bg-grid border border-border p-12 text-center">
          <p className="label-red mb-4">No data yet</p>
          <h3 className="text-2xl font-black uppercase tracking-tight text-ink mb-3">
            Upload your first dataset.
          </h3>
          <p className="text-sm text-ink-faint mb-8 max-w-sm mx-auto">
            Upload a CSV file and AnalyticaAI will automatically profile, analyze,
            and visualize it within seconds.
          </p>
